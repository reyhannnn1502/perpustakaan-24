...
export class LibraryController {
  ...
  filterBooksByYear(year) {
    if (year) {
      this.filteredBooks = this.books.filter((book) => book.year === year);
    } else {
      this.filteredBooks = this.books; // Kembalikan ke semua buku jika filter kosong
    }
    this.renderBooks();
  }
  
  renderBooks() {
    // Render ulang buku yang telah difilter atau semua buku
